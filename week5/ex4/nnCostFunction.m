...
for i = 1:m
	...
	X_m = [ones(1) X(i,:)];
	A1 = sigmoid(X_m * Theta1');
	A1 = [ones(1) A1];
	h_i = sigmoid(A1 * Theta2');
	
	J_m = sum( -y_i .* log(h_i) - ( 1 - y_i) .* log(1-h_i) );	
	J = J + J_m;
...
for i = 1:m
	...
	delta_accum1 = delta_accum1 + delta2(2:end)' * a1;
	delta_accum2 = delta_accum2 + delta3' * [ones(1) a2];
end;
